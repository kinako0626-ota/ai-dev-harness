...
> **Note:** このリポジトリは個人の開発用ツールです。自分のプロジェクトで Claude Code を使う際に、品質を担保するためのハーネスを素早く構築する目的で作成・運用しています。
...
```mermaid
flowchart TD
    START["/implement-team ARC-015 ARC-016 ARC-017"]
    START --> A["Agent A: ARC-015<br>(独立コンテキスト)"]
    START --> B["Agent B: ARC-016<br>(独立コンテキスト)"]
    START --> C["Agent C: ARC-017<br>(独立コンテキスト)"]
    A --> DONE[全タスク完了]
    B --> DONE
    C --> DONE
    DONE --> CR[/code-review]
...
flowchart LR
    ...
    COMMIT --> HOOK[PreToolUse Hook 発火]
    HOOK --> ANALYZE["{{ANALYZE_CMD}} 自動実行"]
    ANALYZE -->|失敗| ABORT[コミット中止]
    ANALYZE -->|成功| OK[コミット実行]
```
...
```mermaid
graph TD
    A[harness.yaml<br>設定ファイル] -->|init.sh| B[Generated Files]

    subgraph B[生成されるファイル群]
        C[CLAUDE.md<br>指示書]
        D[.claude/settings.json<br>フック・環境変数]
    end

    C --> E[Skills<br>7 種のスキル定義]
    D --> F[Pre-commit Hook<br>analyze 自動実行]

    subgraph G[Claude Code Agent]
        H[Implement Agent]
        I[Review Agents<br>3 独立レビュアー]
        J[Fix Agent]
        I --- I1[Convention Reviewer]
        I --- I2[Quality Reviewer]
        I --- I3[Test Coverage Reviewer]
    end

    E --> G
    F --> G

    subgraph K[Reference Files]
        K1[".claude/rules/*<br>プロジェクト規約"]
        K2["docs/conventions/*<br>コーディング規約"]
        K3["docs/plan/*<br>タスク・進捗"]
        K4["docs/reviews/*<br>レビュー結果"]
    end
